...
def test_sse_ambiguous_path_returns_clarification_and_skips_database_nodes() -> None:
    database = SQLiteAdapter("demo", str(ROOT / "data" / "demo.sqlite"))
    graph = build_query_graph(
        database_executor=database,
        llm_client=FakeLLM([]),
    ...
    assert nodes == ["intent_gate", "clarification_answer", "finalize"]
    assert result[-1][1]["intent"] == "clarify"
