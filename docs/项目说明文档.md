...
`classify_intent` 是不访问数据库的入口节点。实现采用规则优先、LLM 兜底：规则只处理高置信的明确数据查询和明确通用问题，边界问题才调用 LLM；LLM 置信度低于 `INTENT_CONFIDENCE_THRESHOLD`（默认 0.75）时统一进入通用回答分支。该节点不携带 Schema、数据库连接串、查询结果或其他敏感数据。
...
| 节点 | 输入 | 输出 | 失败处理 |
| --- | --- | --- | --- |
| `classify_intent` | 用户问题 | `intent`、`intent_reason`、`intent_confidence`、`intent_source` | `general_chat` 直接回答 |
...
- `classify_intent` 识别为 `data_query` 后进入 `retrieve_schema`；`general_chat` 进入通用回答分支；
...
`general_chat` 不读取数据库 Schema，也不检索业务知识库。SQL 修复阶段复用首轮固定的知识库命中和 Schema 上下文，避免同一请求的业务口径发生变化。
