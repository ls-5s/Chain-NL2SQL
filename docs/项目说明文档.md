...
Schema-RAG 的当前代码实现、配置和测试说明见：[Schema-RAG实现说明](Schema-RAG实现说明.md)。单账号认证、跨设备分析会话、上下文记忆与结果引用见：[单账号分析会话实现说明](单账号分析会话实现说明.md)。
...
当前仓库已实现文档上传、解析、切分、SQLite 持久化、异步索引、FTS5/BM25 检索、Graph Prompt 注入和知识命中响应。详细目录、API、任务状态和前端行为见[业务知识库 RAG 实现说明](业务知识库RAG实现说明.md)。

#### 目标使用范围

知识库检索是查询请求的可选增强节点，不新增独立知识问答分支：
...
`general_chat` 不读取数据库 Schema，但可以使用知识片段作为不可信业务背景。SQL 修复阶段复用首轮固定的知识库命中和 Schema 上下文，避免同一请求的业务口径发生变化。
...
- 知识库是全局资源，所有已登录用户可读；仅 `super_admin` 可上传和删除。
...
前端不直接连接数据库、不执行 SQL、不拼接 Prompt。当前 `AgentView` 已接入登录会话、服务端会话列表/详情、SSE 恢复和结果行引用；前端客户端必须兼容当前 SSE 查询接口。Vitest 覆盖认证与会话状态，Playwright 仍需在依赖树恢复后完成端到端验收。
...
当前知识库页面支持 TXT、Markdown、PDF、DOCX、CSV 文件的类型与大小校验、真实异步解析/索引状态、资料元数据查看、轮询和删除；已索引资料以受限命中摘要形式补充业务规则、指标口径和数据字典。知识库是全局资源，不替代按 `database_id` 隔离的 Schema-RAG，也不改变 SQL 安全校验与数据访问策略。
