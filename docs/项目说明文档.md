...
>
> 文档性质：实现与设计说明。已实现能力以当前代码和测试为准；仍标记为“规划中”的内容不属于当前验收范围。
...
只读 SQL 并不等于允许返回全部数据。执行前还要按调用方身份和 `database_id` 应用数据访问策略：表白名单、字段白名单/掩码和可选的行级过滤必须在 SQL 执行前生效；`result_formatter` 是适配器的第一层结果边界，`result_guard` 在 Graph 中执行第二次确定性复核。后者根据已验证 SQL 的 AST 推导输出列，覆盖直接列、别名、表达式、CTE 和派生查询中的敏感字段；字段级权限开启时拒绝 `SELECT *`/`table.*`，结果形状或投影无法证明安全时清空结果并设为 `blocked`。P0 本地 Demo 使用固定的允许表和脱敏字段清单，P1 再接入调用方到数据库权限的映射。
...
前端不直接连接数据库、不执行 SQL、不拼接 Prompt。当前 `AgentView` 已接入登录会话、服务端会话列表/详情、SSE 恢复、澄清后数据库绑定和结果行引用；`RagView` 支持管理员 ACL 配置。Vitest 和 Playwright 完整流程均已通过，详见[Agent完整流程测试说明](Agent完整流程测试说明.md)。
...
Schema-RAG 的元数据加载、ChromaDB、BM25、Hybrid、Reranker、权限过滤、索引版本管理和 Schema 漂移校验已实现；MySQL 适配和数据库注册链路也已接入。CSpider/Spider 批量评测和单轮基线仍待完成。

验收标准：可以重复运行评测，输出 EX、迭代轮次、耗时和失败分类，并能通过开关完成检索策略消融。

### P2：扩展交互能力

实现 Human-in-the-Loop SQL 审批、Example-RAG Few-shot 检索和 Vue3 + Element Plus 前端链路展示；业务知识库 RAG 已作为全局增强能力落地。
...
验收标准：审批前 SQL 不执行；知识库资料能完成上传、解析、切分、索引、检索和删除；检索失败时主 Schema-RAG 流程仍可运行；前端能够展示每轮状态、知识命中来源、SQL、错误和最终结果；扩展功能不改变 P0 API 契约。
